...
class SwipeMap extends Component {
    render() {
        const oneItem = this.props.item.map((item)=> {
            return <ItemDetails item={item} key={item.id} user={this.props.user} goToNext={this.props.goToNext} authentication={this.props.authentication} counting={this.props.counting} />
        });
